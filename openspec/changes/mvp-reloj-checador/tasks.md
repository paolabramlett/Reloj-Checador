...
- [ ] 10.4 Entorno de producción: Supabase prod, migraciones por CI, dominio, monitoreo básico de errores
  ...
  - [x] Migraciones por CI: workflow de GitHub Actions creado en `.github/workflows/supabase-migrations.yml` (corre `supabase db push` al mergear cambios en `supabase/migrations/**`) — **pendiente**: Paola debe agregar el secret `SUPABASE_DB_URL` en GitHub (Settings → Secrets and variables → Actions) para que el workflow funcione, el asistente no tiene acceso a secretos de GitHub
  - [ ] Monitoreo de errores: pendiente que Paola cree cuenta en Sentry y comparta el DSN (no delegable — creación de cuentas está fuera de lo que el asistente puede hacer)
